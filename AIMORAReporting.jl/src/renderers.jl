...
function _tex_block(block::KeyValueBlock)
    rows = join(
        ("$(_tex_escape(key)) & $(_tex_escape(_display_value(value))) & $(_tex_escape(get(block.units, key, ""))) \\\\" for (key, value) in block.entries),
        "\n",
    )
...
function _tex_block(table::ReportTable)
    ...
    rows = join(
        (join(_tex_escape.(_display_value.(row)), " & ") * " \\\\" for row in table.rows),
        "\n",
    )
...
function _render_tikz(figure::FigureBlock)
    ...
    for series in figure.plot.series
        coordinates = join(
            ("($(@sprintf("%.17g", x)),$(@sprintf("%.17g", y)))" for (x, y) in zip(series.x, series.y) if isfinite(x) && isfinite(y)),
            " ",
        )
...
function render_bundle(
    ...
    ispassing(quality) || throw(ArgumentError(
        "report QA failed: " *
        join((issue.message for issue in quality.issues if issue.severity in (:error, :fatal)), "; "),
    ))
